...
- **User-built combined columns (#339)** — set `columnCombining` to let end-users create their own combined columns at runtime: each column's ⋮ menu gains **"Combine columns…"**, which opens an editor to pick other columns to fold into that one (they show in its cell and their own columns hide), choose inline/stacked layout + labels, and **Uncombine** to restore. The result matches a declarative `column.combine` and persists via `stateKey`.
- **Batch edit** — with `checkboxSelection`, selecting rows shows an **Edit** button in the selection toolbar (when editable columns exist). It opens a panel to set one or more columns at once and **Apply** them to every selected row; fires `onBatchUpdate(changedRows, patch, selectedKeys, detail)` plus `onRowsChange`.s` (or `editType: "select"`) edit via a dropdown; number columns get a number input. On commit the grid calls `onRowUpdate(updatedRow, originalRow, field)`; pass `onRowsChange` to also receive the full next rows array (client mode).
