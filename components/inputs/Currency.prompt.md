...
`code`, `value`/`defaultValue`, `onValueChange(number, formatted)` — use this for numeric state; `onChange`
is a native-event escape hatch whose `e.target.value` is a string — plus field props (`label`, `hint`, `error`).
...
`onValueChange` always stays within `number | null` (a transient `-`/`-.` emits `null`, never `NaN`), and
it also fires when a controlled `value`/`precision` change forces the display to re-clamp, so the parent can
reconcile. `disabled` shows `cursor: not-allowed`; `readOnly` renders a sunken, still-selectable state.
