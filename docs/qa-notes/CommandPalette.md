...
- [ ] **[P2] Body scroll not locked while open** — The overlay is a fixed full-viewport scrim, but `<body>` is not scroll-locked, so the page behind the palette can still scroll (wheel after the pointer leaves the panel; iOS body scroll). _Fix:_ set `body { overflow: hidden }` on open / restore on unmount (SSR-guarded), or document as consumer responsibility. `CommandPalette.jsx:64-70`
